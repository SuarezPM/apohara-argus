...
use argus_core::{OrgSummary, WeeklyBriefing, OffenderSummary, TeamSummary};
...
impl LensRunner {
    ...
    pub async fn generate_briefing(
        ...
        let prs_text = prs.iter().map(|p| p.to_string()).collect::<Vec<_>>().join("\n");
        let prompt = format!(
    ...
    pub fn aggregate(
        ...
        let avg_risk = if prs.is_empty() { 0.0 } else {
            prs.iter().map(|p| p.risk_score).sum::<f32>() / prs.len() as f32
        };
        let critical_findings = prs.iter()
            .filter(|p| p.critical_findings > 0)
            .count() as u32;
        ...
        let by_team: Vec<TeamSummary> = by_author.iter().map(|(team, prs)| {
        ...
        let top_offenders: Vec<OffenderSummary> = prs.iter()
    ...
    pub async fn run(
        ...
        let prs_summary = prs.iter()
            .map(|p| format!("- PR {} by {}: risk {:.2}, top finding: {}",
        ...
        Ok(LensOutput { briefing, org_summary, markdown })
    }
}
...
impl std::fmt::Display for PRBriefSummary {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "- PR {} by {}: risk {:.2}, top finding: {}", self.pr_ref, self.author, self.risk_score, self.top_finding)
    }
}
...
fn render_markdown(b: &WeeklyBriefing, org: &OrgSummary, prs: &[PRBriefSummary]) -> String {
    ...
    s.push_str(&format!("Critical findings: **{}**\n\n", b.critical_findings));
    ...
    s.push_str(&format!("{} PRs analyzed, slop score {:.2}, fit score {:.2}*\n",
...
mod tests {
    ...
    fn aggregate_empty() {
        ...
        let (b, o) = r.aggregate("acme", chrono::NaiveDate::from_ymd_opt(2026, 6, 10).unwrap(), &[]);
    ...
    fn aggregate_with_data() {
        ...
        let (b, o) = r.aggregate("acme", chrono::NaiveDate::from_ymd_opt(2026, 6, 10).unwrap(), &prs);
