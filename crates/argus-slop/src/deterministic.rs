...
impl SlopSignal {
    pub fn info(rule_id: &str, line: usize, msg: impl Into<String>) -> Self {
        Self { rule_id: rule_id.to_string(), severity: Severity::Info, line, message: msg.into() }
    }
    pub fn warning(rule_id: &str, line: usize, msg: impl Into<String>) -> Self {
        Self { rule_id: rule_id.to_string(), severity: Severity::Warning, line, message: msg.into() }
    }
    pub fn error(rule_id: &str, line: usize, msg: impl Into<String>) -> Self {
        Self { rule_id: rule_id.to_string(), severity: Severity::Error, line, message: msg.into() }
    }
}
                    ...
                    if let (Some(_start), Some(name)) = (body_start, fn_name.take()) {
                        if body_lines > OVERSIZED_FN_LOC {
                            signals.push(SlopSignal::warning(
                                "SLOP-001",
                                fn_start_line,
                                format!("Function '{}' has {} LOC (> {})", name, body_lines, OVERSIZED_FN_LOC),
                            ));
                        }
                    }
                    body_start = None;
...
mod tests {
    ...
    fn oversized_function() {
        let body = (0..100).map(|i| format!("    let _x{i} = {i};")).collect::<Vec<_>>().join("\n");
        let src = format!("fn big() {{\n{body}\n}}\n");
        let signals = run_deterministic_rules(&src);
        assert!(signals.iter().any(|s| s.rule_id == "SLOP-001"), "got: {:?}", signals);
    }
    ...
    fn swallowed_error() {
        ...
        let signals = run_deterministic_rules(src);
        assert!(signals.iter().any(|s| s.rule_id == "SLOP-002"), "got: {:?}", signals);
    }
    ...
    fn unwrap_in_test_ok_outside_warns() {
        ...
        assert!(!ok.iter().any(|s| s.rule_id == "SLOP-004"), "test should not warn: {:?}", ok);
        assert!(bad.iter().any(|s| s.rule_id == "SLOP-004"), "prod should warn: {:?}", bad);
    }
    ...
    fn todo_stub() {
        let src = "fn f() {\n    // TODO: implement\n    let _ = 1;\n}\n";
        let signals = run_deterministic_rules(src);
        assert!(signals.iter().any(|s| s.rule_id == "SLOP-003"), "got: {:?}", signals);
    }

    #[test]
    fn performance_10k_loc_under_500ms() {
        let body = (0..10_000).map(|i| format!("    let _x{i} = {i};")).collect::<Vec<_>>().join("\n");
        let src = format!("fn big() {{\n{body}\n}}\n");
