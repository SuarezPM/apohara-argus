...
impl Analyzer for VerdictSynthesizer {
    ...
    fn name(&self) -> &'static str { "aegis-verdict" }
    fn prompt_name(&self) -> &'static str { "verdict-synthesizer" }
    ...
    fn parse_response(&self, raw: &str) -> Result<Verdict, SlopError> {
        ...
        let resp: SynthesizerResponse = serde_json::from_str(&json_str)
            .map_err(|e| SlopError::Parse(e.to_string()))?;
