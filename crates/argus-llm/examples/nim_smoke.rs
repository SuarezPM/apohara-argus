...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let key = env::var("ARGUS_NIM_KEY")
        .expect("Set ARGUS_NIM_KEY env var (your NVIDIA NIM BYOK key)");
    if key.is_empty() {
        eprintln!("ARGUS_NIM_KEY is empty. Get a free key at https://build.nvidia.com/");
        std::process::exit(1);
    }

    let model = env::var("ARGUS_NIM_MODEL")
    ...
    let resp = client.complete_one_shot(&model, sys, user, &key, 0.0, 64).await?;
